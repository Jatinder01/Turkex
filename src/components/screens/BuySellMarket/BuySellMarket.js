...
const BuySellMarket = (props) => {
  ...
  //   { key: "first", title: strings.buy_sell_market.order_book },
  //   { key: "second", title: strings.buy_sell_market.market_trades },
  // ]);
  const dispatch = useDispatch();

  // console.log("selectedCoinPair=-=-=-=->>>>", selectedCoinPair);
        ...
        if (isLogin == "true") {
          ...
          dispatch(getFavMarketData())
            .then((res) => {
              ...
              if (arr?.length > 0) {
                let pair = `${selectedCoinPair?.base_unit?.toUpperCase()}${selectedCoinPair?.quote_unit?.toUpperCase()}`;

                let status = res.find((val) => val === pair.toLowerCase());
  ...
  const hasAndroidPermission = async () => {
    if (Platform.OS === "android") {
      ...
      if (status === "granted") {
        ...
        // Singleton.getInstance().showError(strings.buy_sell_market_screen.camera_permission_denied);
  ...
  //       source={{
  //         uri: "https://stage-evoeurope.s3.us-east-2.amazonaws.com/trading2/main.html?market=",
  //       }}
  //     />
  //   </View>
  // );
              ...
              .then((isLogin) => {
                ...
                if (isLogin == "true") {
                  dispatch(updateFavMarketData(pairName))
                    .then((res) => {
                      console.log("updateFavMarketData=-ee=-res33>>>", res);
                    ...
                    .catch((err) => {
                      console.log("updateFavMarketData=-err=33->>>", err);
                    });
                  console.log("selectedCoinPair=-=-=", selectedCoinPair);
                }
              });
