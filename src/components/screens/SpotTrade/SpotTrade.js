...
const SpotTrade = (props) => {
  ...
  const [pairIdTrade, setPairIdTrade] = useState(
    orderHistoryReducer?.marketCoinInfo[0].id
  );
  ...
  const arrPairTrade = [...orderHistoryReducer?.marketCoinInfo];
  const [pairFilterTrade, setpairFilterTrade] = useState(
    orderHistoryReducer?.marketCoinInfo[0].name
  );
  ...
  const [mainFromDate, setMainFromDate] = useState(new Date());
  const [mainToDate, setMainToDate] = useState(new Date());
  ...
  const [itemsTrade, setItemsTrade] = useState([
    ...orderHistoryReducer?.marketCoinInfo,
  ]);
  //eg
  const [pairToggle, setPairToggle] = useState(false);
  const [pairToggleTrade, setPairToggleTrade] = useState(false);

  const [stateToggle, setStateToggle] = useState(false);
  const [typeToggle, setTypeToggle] = useState(false);
  const [orderToggle, setOrderToggle] = useState(false);
  const [pairToggleIndex, setPairToggleIndex] = useState(0);
  const [pairToggleTradeIndex, setPairToggleTradeIndex] = useState(0);
            ...
            >
              {fromDateSelected ? (
                <DatePicker
                  ...
                  // open={showDatePicker}
                  mode="date"
                  date={fromDate ? mainFromDate : date1}
                  maximumDate={date1}
                ...
                <TouchableOpacity
                  onPress={() => {
                    ...
                    dispatch(cancelAllOrderWithoutParams())
                      ...
                      .catch((err) => alert(constants.APP_NAME_CAPS, err));
                  }}
                  style={{
                    backgroundColor: ThemeManager.colors.selectedTextColor,
                    justifyContent: "center",
                ...
                */}
                ...
                <View>
                  ...
                  <ToggleSwitch
                    ...
                    onPressItem={(val, index) => {
                      console.log("=-=pair-=-=->>>>>-", val, index);
                      console.log("arrPairTrade=-=-=-=>>>>", arrPairTrade);
                      // let replaceVal = val.replace("/", "");
                      setPairToggleTradeIndex(index);
                      // if (index == 0) {
                      //   setPairIdTrade('');
                      //   // selectedIndex(index)
                      // } else {
                      setPairIdTrade(val.id);
                      // selectedIndex(index)
                      // }
                      setpairFilterTrade(val.id);
                      // setPairToggle(false);
                    }}
                    list={arrPairTrade}
                    selectedIndex={pairToggleTradeIndex}
                  />
                  <BorderLine />
                  {/* <SelectDropdown
                    ref={pairDD}
                ...
                >
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      setpairFilter("All");
                      setpairFilterTrade(arrPairTrade[0].name);
                      setStateFilter("All");
                      settTypeFilter("All");
                      setOrderTypeFilter("All");
                      setfromDate("YYYY-MM-DD");
                  ...
                  <ButtonPrimary
                    title={strings.spot.confirm}
                    onPress={() => {
                      console.log("pairIdTrade=-=-=-=", pairIdTrade);
                      let linkConcat = "";
                      dispatch(resetTradeHistory());
                      if (pairFilterTrade != "All") {
                        linkConcat = linkConcat + "&market=" + pairIdTrade;
                      }
